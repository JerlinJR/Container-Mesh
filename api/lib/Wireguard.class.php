...
class Wireguard{
    ...
    public function addPeer2($public, $email, $reserved, $ip = null)
    {
        if (!$this->hasPeer($public)) {
            $ipnet = new IPNetwork($this->getCIDR(), $this->device);
            $next_ip = $ipnet->getNextIP($email, $ip);
            ...
            $next_ip = escapeshellarg($next_ip);
    
            $cmd = "sudo wg set $this->device peer \"$public\" allowed-ips $next_ip 2>&1";
            $output = [];
            $result = null;
            exec($cmd, $output, $result);
    
            $cmdSave = "sudo wg-quick save $this->device 2>&1";
            $outputSave = [];
            $resultSave = null;
            exec($cmdSave, $outputSave, $resultSave);
    
            if ($result === 0 && $resultSave === 0) {
                return $ipnet->allocateIP($next_ip, $email, $public, boolval($reserved));
            } else {
                // Log or return the command output and exit codes for debugging
                return "Command 1 Output: " . implode("\n", $output) . "\nCommand 1 Exit Code: $result\nCommand 2 Output: " . implode("\n", $outputSave) . "\nCommand 2 Exit Code: $resultSave";
            }
        } else {
            throw new Exception("Peer already exists");
        }
